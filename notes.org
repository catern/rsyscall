...
** remove binaries from standardtask
...
*** TODO figure out pkglibexec things
    OK, so.
...
** use a union for repl results, not inheritance
...
are there any crates/examples of a more safe API for readers than std::io::Read? returning the number of bytes read is certainly very traditional and familiar, but one could mess up the usage of that number and read too many or too few bytes out of the buffer one passed to read(); 
