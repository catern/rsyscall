...
** remove syscall eof message
   ...
   Exit it?
** optimize ssh startup
   It's annoyingly slow. 100 milliseconds! And I run it three times for each ssh!

   The obvious optimization is multiplexing...
** TODO remove need for careful use of unshare_files with persistent thread
   In fact there's a ton of crazy unshare_files stuff going on here.
** TODO fix mysterious __del__ exceptions
