...
** TODO don't actually read the memory when we read signalfd in childtaskmonitor
...
** TODO remove binaries from standardtask
...
*** TODO figure out temporary directory cleanup
    blah blah argh blah
...
** make far.Process into a handle
...
** revamping memory management for shared memory
*** stick MemoryMapping directly into all pointers
*** start managing pointers in file_to_allocation_to_handles
...
Oh so hmm. We could have three levels: process, child process (which allows waitid and tracks usage), and threadprocess (which further holds stack and TLS notifications, and can be released to child process).
...
What would be great is if I could just wait on sigchild without worrying if I had an event. Hmm. Couldn't I do that by optimistically waiting, then sigchild waiting if I don't get anything? Hm.
...
** split out both spawn_exec and fork task creation
...
** change child_task_monitor to issue waitids for each process
...
** remove usage of far for syscalls
   I notice immediately, set_robust_list
** re-optimize do_cloexec_except
