...
** CLOEXEC also affecting unshare
   wait, wait, can't we just do the closing ourselves after the unshare?

   well, again, in practice that would require a scan of the fd table

   what I'd really like is some way to instead, mark an fd as inherited?
   no, pass them as an argument?
   no, have them inherit by default?
   no, have both?

   we need to be able to hold arbitrary fds after the unshare,
   and simultaneously support dynamic scoping of fds (so something is inherited unknowingly)

   I think essentially we want unshare to take a list of fds which are inherited,
   and also inherit anything not marked CLOEXEC
