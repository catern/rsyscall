...
** convert serializer to tagless final style
...
** convert socketpair and pipe (and other calls returning fds in memory) to handle style
...
as an alternative, I could just write overloads.

but, it would be nice to split out the implementation into different places.
...
well it's not typechecked which is awkward

Oh, I can fall back to that. yeah! simple!

ok lol I don't need this, I can just special-case bytes

and I can have serializable always take the Task... er wait no

that's a circular depppp
