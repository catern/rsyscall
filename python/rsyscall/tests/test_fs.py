...
class TestFS(TrioTestCase):
    ...
    async def test_fdat(self) -> None:
        ...
        root = await self.process.task.open(await self.process.ptr(self.tmpdir), O.DIRECTORY)
        name = await self.process.ptr("foo")
        await root.mkdirat(name)
        await root.rmdirat(name)
        file = await root.openat(await self.process.ptr("."), O.TMPFILE|O.WRONLY)
        await self.process.task.linkat(None, await self.process.ptr(file.as_proc_path()), root, name, AT.SYMLINK_FOLLOW)
