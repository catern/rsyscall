...
async def do_async_things(self: unittest.TestCase, epoller: Epoller, thr: RAMThread, i: int=0,
    ...
    async def stuff():
        logger.info("do_async_things(%d): read: starting on %s", i, async_pipe_rfd.handle.near)
        result = await async_pipe_rfd.read_some_bytes()
        logger.info("do_async_things(%d): read: returned", i)
    ...
    async with trio.open_nursery() as nursery:
        ...
        logger.info("do_async_things(%d): write: starting on %s", i, async_pipe_wfd.handle.near)
        await async_pipe_wfd.write_all_bytes(data)
        logger.info("do_async_things(%d): write: returned", i)
        await stuff()
