...
async def read(task: Task, fd: FileDescriptor, buf: Pointer, count: int) -> int:
    ...
    return (await near.read(task.sysif, fd.near, buf.near, count))

async def write(sysif: SyscallInterface, fd: FileDescriptor, buf: Pointer, count: int) -> int:
    assert task.fd_table == fd.fd_table
    assert task.address_space == buf.address_space
    return (await near.write(task.sysif, fd.near, buf.near, count))
