...
async def start_powerdns(nursery, parent: Process, path: Path, zone: dns.zone.Zone,
    ...
    ipv4s = {str(i+1): (udp.move(process.task), tcp.move(process.task))
             for i, (udp, tcp) in enumerate(ipv4_sockets)}
    ipv6s = {str(i+1): (udp.move(process.task), tcp.move(process.task))
             for i, (udp, tcp) in enumerate(ipv6_sockets)}
    await process.unshare_files(going_to_exec=True)
    config = {
        ...
        # backend
        "launch": "bind",
        "bind-config": os.fsdecode(await process.spit(path/"named.conf",
            'zone "%s" { file "%s"; };' % (
                zone.origin.to_text(),
                os.fsdecode(await process.spit(path/"zone", zone.to_text()))))),
    ...
    child = await process.exec(pdns_server.args(*[f"--{name}={value}" for name, value in config.items()]))
...
async def start_recursor(nursery, parent: Process, path: Path,
    ...
    if root_hints is not None:
        config["hint-file"] = os.fsdecode(await process.spit(path/'root.hints', root_hints.to_text()))
    child = await process.exec(pdns_recursor.args(*[f"--{name}={value}" for name, value in config.items()]))
    nursery.start_soon(child.check)
    return Powerdns()
...
class TestPowerdns(TrioTestCase):
    async def asyncSetUp(self) -> None:
        self.process = await local.process.clone()
        await self.process.unshare_user(0, 0)
        await self.process.unshare(CLONE.NEWNET)
        # set loopback up
        ip = await self.process.environ.which("ip")
        ...
        # no that's silly
        await self.process.run(ip.args('link', 'set', 'dev', 'lo', 'up'))
        self.pdns_idx = 1
        self.tmpdir = await self.process.mkdtemp("test_powerdns")
        self.path = self.tmpdir.path
        self.dig = await self.process.environ.which("dig")
        self.host = await self.process.environ.which("host")
    ...
    async def start_authoritative(self, zone: dns.zone.Zone) -> ipaddress.IPv4Address:
        ...
        sockaddr = await self.process.ram.ptr(SockaddrIn(53, addr))
        print("sockaddr", sockaddr)
        path = await self.process.mkdir(self.path/f"pdns{idx}")

        udp_sock = await self.process.task.socket(AF.INET, SOCK.DGRAM)
        await udp_sock.bind(sockaddr)
        tcp_sock = await self.process.task.socket(AF.INET, SOCK.STREAM)
        ...
        powerdns = await start_powerdns(
            self.nursery, self.process, path, zone, [(udp_sock, tcp_sock)], [])
        return addr
    ...
    async def start_recursor(self, root_hints: dns.zone.Zone) -> ipaddress.IPv4Address:
        ...
        powerdns = await start_recursor(
            self.nursery, self.process, path, [(udp_sock, tcp_sock)], [], root_hints=root_hints)
        return addr
    ...
    async def test_basic(self) -> None:
        ...
        await self.process.run(self.host.args('a.user', str(recursor_addr)))
    ...
    async def test_bad_ns(self) -> None:
        ...
        await self.process.run(self.dig.args('@'+str(magic_addr), 'NS', 'magical'))
...
"return table.concat(nslabels, '.');"
        ...
        # await self.process.run(dig.args('@127.0.0.2', 'NS', 'a.root-servers.net'))
        # await self.process.run(self.dig.args('@'+str(user_addr), 'A', 'a.user.1.magic'))
        # await self.process.run(self.dig.args('@'+str(recursor_addr), '+trace', 'NS', 'user'))
        # await self.process.run(self.dig.args('@'+str(recursor_addr), 'A', 'a.user.1.magic'))
        await self.process.run(self.dig.args('@'+str(magic_addr), 'NS', 'a.user.1.magic'))
        # await self.process.run(self.dig.args('@'+str(recursor_addr), 'A', 'user'))
        # await self.process.run(self.dig.args('@'+str(recursor_addr), 'A', 'a.user.1.magic'))
        # await self.process.run(self.host.args('-v', '-a', 'a.user.1.magic', str(recursor_addr)))
        # await self.process.run(self.host.args('-a', 'a.user.1.magic', str(magic_addr)))
        # raise Exception('hi')
