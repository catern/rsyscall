...
@dataclass
class Miredo:
    # we could use setns instead of keeping a process around inside the namespace.
    ...
    # So, I'll just use a process, which I do understand.
    # Hopefully we can get a more lightweight setns-based approach later?
    ns_process: ChildProcess
...
async def set_miredo_sockopts(process: Process, fd: FileDescriptor) -> None:
    # set a bunch of sockopts
    one = await process.ram.ptr(Int32(1))
    ...
    await fd.setsockopt(SOL.IP, IP.MTU_DISCOVER, await process.ram.ptr(Int32(IP.PMTUDISC_DONT)))
...
async def unmount_everything_except(process: Process, path: Path) -> None:
    pass

async def start_miredo_internal(process: Process, miredo_exec: MiredoExecutables) -> Miredo:
    ### create socket outside network namespace that Miredo will use for internet access
    inet_sock = await process.socket(AF.INET, SOCK.DGRAM)
    await inet_sock.bind(await process.ptr(SockaddrIn(0, 0)))
    # set some miscellaneous additional sockopts that Miredo wants
    await set_miredo_sockopts(process, inet_sock)
    ### create main network namespace process
    ns_process = await process.clone(CLONE.NEWNET|CLONE.NEWUSER)
    ### create in-network-namespace raw INET6 socket which Miredo will use to relay pings
    icmp6_fd = await ns_process.socket(AF.INET6, SOCK.RAW, IPPROTO.ICMPV6)
    ### create in-network-namespace socket which Miredo will use for unassociated Ifreq ioctls
    reqsock = await ns_process.socket(AF.INET, SOCK.STREAM)
    ### create and set up the TUN interface
    tun_fd, tun_index = await make_tun(ns_process, "miredo", reqsock)
    ...
    privproc_side = privproc_process.inherit_fd(privproc_pair.first)
    await privproc_side.dup2(privproc_process.stdin)
    await privproc_side.dup2(privproc_process.stdout)
    privproc_child = await privproc_process.execve(miredo_exec.privproc.executable_path, [
    ...
    client_process = await ns_process.clone(CLONE.NEWUSER|CLONE.NEWNET|CLONE.NEWNS|CLONE.NEWPID)
    # lightly sandbox by unmounting everything except for the executable and its deps (known via package manager)
    await unmount_everything_except(client_process, miredo_exec.run_client.executable_path)
    # a helper function
    async def pass_fd(fd: FileDescriptor) -> str:
        await client_process.inherit_fd(fd).disable_cloexec()
        return str(int(fd))
    client_child = await client_process.execve(miredo_exec.run_client.executable_path, [
    ...
    return Miredo(ns_process, client_child, privproc_child)

async def start_miredo(nursery, miredo_exec: MiredoExecutables, process: Process) -> Miredo:
    miredo = await start_miredo_internal(process, miredo_exec)
...
class TestMiredo(TrioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        print("a1", time.time())
        self.miredo = await start_miredo(self.nursery, self.exec, self.process)
        print("b", time.time())
        self.netsock = await self.miredo.ns_process.task.socket(AF.NETLINK, SOCK.DGRAM, NETLINK.ROUTE)
        print("b-1", time.time())
        print("b0", time.time())
        await self.netsock.bind(
            await self.miredo.ns_process.ram.ptr(SockaddrNl(0, RTMGRP.IPV6_ROUTE)))
        print("b0.5", time.time())


    async def test_miredo(self) -> None:
        print("b1", time.time())
        ping6 = (await self.process.environ.which("ping")).args('-6')
        ...
        await add_to_ambient_caps(process, {CAP.NET_RAW})
        await (await process.exec(ping6.args('-c', '1', 'google.com'))).check()
        print("d", time.time())
