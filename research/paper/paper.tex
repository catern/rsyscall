...
This can be mitigated to some degree through sytem calls like \texttt{vfork},
but inflicts further complexity on the caller.\cite{vfork}\cite{vfork_dangerous}
...
\lstinputlisting[float,language=Python,label={basic},caption={Create new process, change CWD, and exec}]
...
\lstinputlisting[float,language=Python,label={pipe},caption={Creating a concurrent processing pipeline}]
...
The Filesystem in Userspace (FUSE) Linux subsystem
allows a filesystem to be implemented with a userspace program.\cite{fuse}
...
Baumann (2019) briefly suggests cross-process operations as a possible replacement for fork on Unix,
...
examples include Exokernel and Fuschia.\cite{exokernel}\cite{fuschia_libc}
...
have process capabilities which allow one process to operate on another process.\cite{keykos}\cite{sel4}\cite{fuschia}
