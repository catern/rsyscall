...
\item
  Does our \texttt{rsyscall}-based Python direct-style process creation interface
  have acceptable performance relative to the Python standard library's spawn-style \texttt{subprocess.run}?
...
\end{figure}
Does our \texttt{rsyscall}-based Python direct-style \texttt{clone}
have acceptable performance relative to the Python standard library's spawn-style \texttt{subprocess.run}?
...
We simply run \texttt{getpid} in the child process on the same benchmark setup;
the average time per \texttt{getpid} call is .561 milliseconds.
The \texttt{getpid} syscall returns essentially instantly,
so we can regard this time as the amount of overhead incurred
...
Our work, though developed independently and largely completed before that paper,
