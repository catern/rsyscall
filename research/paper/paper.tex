...
or other similar fork-style interface,
both processes execute concurrently,
and the child process typically inherits all but a few attributes from the parent process.\cite{tlpi}
...
both of which encourage systems programmers to avoid them.\cite{tlpi}\cite{posix_spawn_lwn}
...
\lstinputlisting[float=h,language=Python,label={basic},caption={Create new process, change CWD, and exec}]
...
When a parent process creates a child process by fork,
both processes execute concurrently,
and the child process typically inherits all but a few attributes from the parent process.\cite{tlpi}
The child process can call arbitrary system calls
to mutate the new process until it reaches the desired state,
...
which creates the new process all at once.
Other details, such as security context, are inherited from the parent process.\cite{tlpi}\cite{create_process}
The most well known spawn-style interfaces are Unix's \texttt{posix\_spawn}
and Windows \texttt{CreateProcess};
the Unix shell also serves as a spawn-style process creation interface,
especially when used with Bernstein chaining.\cite{posix_spawn}\cite{create_process}\cite{chainloading}
...
Spawn-style interfaces, by their nature as a single call,
don't allow programs to branch on the results of individual modifications to the new process,
and have complex, or substantially less information, error reporting for such modifications.
\cite{posix_spawn}\cite{posix_spawn_error_pipe}
This further reduces the power of spawn-style interfaces.
