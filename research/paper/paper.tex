...
\texttt{rsyscall} can work entirely in userspace,
...
Thus, this is a form of abstraction,
...
\subsection{Cross-process syscalls}
...
\texttt{clone} creates a new process
...
we use this to call an arbitrary function address stored on the stack.
...
\texttt{clone} without \verb|CLONE_VM| is comparable to fork.
...
We create a child process, exec the \texttt{true} binary, and wait for it to exit
using both the Python standard library's \texttt{subprocess.run},
and direct-style with \texttt{rsyscall}.
...
Direct-style starts being faster than \texttt{subprocess.run}
...
This work, though developed independently and largely completed before that paper,
