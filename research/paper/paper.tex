...
returning the result as a single register-sized integer.
...
which performs the syscall and sends the result back.
...
when not executing a syscall or writing out the result,
...
\texttt{ptrace}\cite{ptrace} already provides an in-kernel way to perform arbitrary actions on a target process,
