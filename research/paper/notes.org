...
**** DONE thicker lines on graph
**** DONE make graph lower height
**** bait and switch?
...
**** DONE ok so again what are we changing about the intro
...
**** DONE background section balance
...
**** DONE other thought
     why not C
     why not kernel
**** DONE let's argue that fork is complex by analogy to continuations
...
*** show different applications using the new primitive
    ...
    well we aren't really doing this, just showing one application.
*** DONE compare to something more optimized than python sub-process
    ...
    ok we compared it, nice
*** DONE more experiements evaluating different aspects of the system
...
*** DONE integration with applications that you can talk about in detail
...
* irc feedback
** DONE move citations before period
** DONE consistent formatting fork and spawn
** DONE have central contributions section
** DONE fix weird alignment in style comparison table
** DONE fix hideous whitespace
section 3.3.2

For \texttt{unshare(CLONE\_FILES)}, we perform \texttt{close\_cloexec} automatically in userspace.

baleeted!
** DONE clarify how local syscalls are made
catern: in Sec. 3.0, I’m missing something on “syscall server”. After going in order through Sec. 2, I still do.
Is the syscall server a separate thread?

quite important!
** DONE discuss clone directly
   and implementing the necessary parts.

   probably should explicitly talk about clone!
   we have space now.
*** better comparison to fork
   why not have fork execute a lambda,
   and have that return a value.

   well that would ultimately regress to, like, sfork.
   run a lambda in the child and return once it's done with exec or exit,
   or raised an exception out.

   maybe i should talk more about the,
   run code in the process to initialize it,
   model.
   that's what the fork model is ultimately,
   and it's awkward.

   the programming model is awkward, the error handling is awkward

   we should probably talk about clone launching a function explicitly.

   yeah launching a function is a good thing to examine.

   that also discusses clone directly.

   we can talk about that in the evaluation?
...
*** fork-style
    ...
    are interfaces where the new process is launched running a function of the caller's choice;
    for example, glibc provides a wrapper over the \texttt{clone} system call
    which allows specifying a function pointer to execute in the new process.\cite{clone}\cite{sfork}\cite{afork}
    The new process may be created sharing its address space with the parent,
    which negates the inefficiency when used from a large memory process,
    with some restrictions.\cite{vfork_dangerous}
    However, the other issues still remain;
    we class these also as "fork-style" interfaces.

    One might imagine a wrapper for process creation
    which automatically propagates exceptions thrown in a child process creation function.
    This would abstract away the error-reporting code we show here.
    But such a wrapper has no obvious implementation in..


    ok why don't we like the wrapper?
    - it doesn't let you coordinate the program
    - it still won't let you work in threads... or will it? yes, I guess it will.
    - it's complex (but that's just because it's concurrent)
    - it doesn't make sense.

    Like how would you implement it?
    it would just be implemented via fork I guess.
    or, by creating a thread, rather.
    like you'd create a thread which runs in the new process.

    that has the vfork issues, sure.
    why does baumann not like stuff
    hmm he doesn't mention the concept.
    thread-safe, fine
    insecure, fine
    all of his things are fine with the thread idea. and he got published! lol.

    maybe we should compare the impls..?
    ...
    We built several such wrappers before building \texttt{rsyscall}.

    Such wrappers don't solve the core issue with fork-style process creation:
    Concurrent communication is complex.
    A wrapper may explicitly support returning errors

    We found that such wrappers, besides their complexity,
    don't provide any fundamental improvement
    for the issue of communication between concurrently executing programs in the child process and parent process.
    ...
    Only direct-style process creation is able to easily express such functionality.

    they don't allow

    ok so we could vfork again I guess...

    don't solve one of the fundamental issues with fork-style process creation:
    Concurrent execution is difficult.
    As such, while 
    Concurrent execution of the parent process and the child process

    Explicitly supported 
    It is difficult for to 

    The primary problem with such wrappers
    is that 

    Such wrappers have two problems:
    Their abstraction obscures the low-level primitive actually available on Linux and is difficult to understand

    which allow launching the new process running a function of the caller's choice,
    as the glibc wrapper for \texttt{clone} does, as mentioned in section \ref{introduction}.
    What we found is that
    such a wrapper cannot solve the core problem with fork-style process creation:
    Concurrent execution of the process creation 

    hmm ugh.
    maybe we should do some fallback behavior...


is there a good adjective that covers normal "concurrency" i.e. two threads concurrently executing, but also covers the
nature of
  running something under a function like bracket?  like, basically I want a more


two concurrently executing threads can't easily interact with each other, not like a single-threaded program which can just call a function and get a return value.
when function A passes a function B to some higher-order function, function B can't easily interact with A, in the same way. but they're not (necessarily) executing concurrently. what's a good word for that?

    which allowed a 
allowing the use of 
    which automatically propagates exceptions thrown in a child process creation function to the parent.
    which automatically propa
    

    One might imagine a heavily abstracted interface,
    which automatically propagates errors from the child process initialization function
    back to the parent process.
    We created such an interface,
    but found that the other issues of fork-style process creation still remained.\cite{sfork}
    Errors could not be handled on an individual basis,
    and the programming model was complex;

    In general, any fork-style interface for process creation ---
    one which runs the process initialization code in a separate context from the main program ---
    h
    a program must
    

    Such interfaces can remove the issues with memory,
    while maintaining the fork-style arbitrary code initializing the new process.

    With sufficient abstraction,
    they can also provide an easier error reporting interface.

    (They can also provide an easier error reporting interface, hm.)

    Such interfaces can provide an easier error reporting interface.

    (hmm if we just let the exception propagate in fork-style then...
    well then we'll have bad things happen when we unwind the stack.
    it's just not safe!)

    However they still don't allow reacting to errors in individual syscalls without IPC,
    they still constrain the parent process to be single-threaded,
    and they still have a complex programming model that doesn't allow coordination with the main program.
    and they still don't all
** include other fork-style implementations in appendix
   mmm okayyyy maybe
** talk about correctness?
   development process, how many lines of code, etc.

   well, maybe not.
   or maybe!

   we could include it in our implementation section

   things we do:
   use types
   plenty of runtime asserts on correctness of references

   reasons it's hard:
   lots of different namespaces for references

   (we have the near/far system for that)
** DONE mention event loops
   justify their presence?

   they appear suddenly in implementation section

   A key design constraint is integration with event loops.
   Some parts of our implementation 
   require waiting for multiple events at the same time;
   we'll discuss one instance in section \ref{execve}.
   This is performed via event loop.
   
   This depends on event loop 
   Furthermore, 

   A key design constraint is integration with event loops.
   While in section \ref{examples} we have only shown a traditional, synchronous API,
   calling a syscall in another process which may block should not require the calling process to block as well.
   Many programming languages, including Python, depend on event loops for their asynchronous, concurrent APIs,
   and so we should 
   Achieving this in typical programming requires integration with 

   Even if a syscall in a target process is synchronous,
   this should not require the calling process to synchronously 
   Calling a syscall in a target process should not require the calling process to block processing of .
   
   and 
   so a key design constraint is integration with event loops.
   we should be able to integrate with an event loop to remove
   We take care to function asynchronously;
   our Python implementation has full support for asynchronous op
** DONE talk about "only one program is in scope"
   "we" means the program controlling multiple processes

   maybe say this in the examples section?

   
** DONE fd gc motivation
<pgiarrusso> catern: now 3.3.2 provides some motivation that was needed earlier
 — "leaving unwanted file descriptors open [...] causes bugs"
<pgiarrusso> that belongs in 3.3.0, methinks

<pgiarrusso> catern: so I got 3.3.2 more than 3.3.1, either because of discussion or motivation
> pgiarrusso: that's a bit unexpected for me, hm, yeah, maybe I will move that line you mentioned from 3.3.2 earlier
<pgiarrusso> catern: that's the only section where I struggled
<pgiarrusso> catern: it sounds like "understanding bias/blindness" or however that's called — you understand this well but...
> yeah, that's exactly what I'm thinking right now
<pgiarrusso> also 3.3.2 _is_ complicated
<pgiarrusso> I'd focus some effort on this section, it seems to need more work
<pgiarrusso> I suspect walking through an example _there_ would help understand the motivation and what is happening
** citations
<pgiarrusso> catern: and last, your biblio has both overflowing links and unwanted lowercase
 like `Standardizing wasi: A system interface to run webassembly outside the web.` — 
you need to manually write  `{WASI}` and `{WebAssembly}` in the source bibtex
<pgiarrusso> (the braces belong to latex, the backquotes are markdown-like formatting)
*** weird linebreaking on USENIX Association cite
** talk about using clone directly
   and how hard that would be
** DONE ok so the fd gc stuff is all
   worthless.

   it's just overcomplicated and bad.

   we should mostly cut it.
   ...
   which...
   does that even need us to mention that we have garbage collected file descriptors?
   honestly, doesn't it only need, um. hmm..

   ok so when creating processes and inheriting file descriptors,
   we need to clean up the file descriptor table.

   that's the only thing that matters.

   but, really, that doesn't have anything to do with direct-style process creation!

   like, nothing at all!
   no-one in existing fork-style cleans up the file descriptor table.
   it's irrelevant!

   ok, so what is novel from direct-style here?

   well, fork isn't. and unshare isn't.

   now that we default to clone_files off, that's not relevant either.
   clone_files is a special case, as before.

   oh hey we can always bulk up by just reverting those style changes :) let's do that now.

   lol ok so now we're at 12 pages

   anyway, so what's the point of this fd section?

   what does direct style bring here?

   well, ok.

   one thing is that,
   we need to be able to operate across multiple processes.
   raw file descriptor numbers are not suitable.

   so using GC'd FD handles is necessary.

   and that's where those methods come from.

   Many Unix resources are referred to by integers,
   which are valid only within the context of a given process;
   file descriptors numbers, memory addresses, and others.
   A program using a POSIX libc, which operates in only one process,
   can deal with these resources by just passing around these integers
   and directly access them through syscalls and instructions.
   A program using \texttt{rsyscall}, however,
   operates in multiple processes,
   and dealing with these resources as raw integers incurs the risk of a new kind of error:
   using the wrong process to try to access a given resource.

   We'll discuss our approach to this problem for file descriptors;
   our approach for other resources is similar.

   We deal with file descriptors only through file descriptor handles.
   A file descriptor handle is a pair of a file descriptor number, and a process capability.

   To perform a system call on a file descriptor through a given process,
   we must obtain a handle for that file descriptor with that process capability.
   There are three ways to do this.

   First, all system calls which, in a POSIX libc, would return a file descriptor number,
   now return file descriptor handles
   consisting of the file descriptor number they would have otherwise returned,
   and the process capability for the process in which the system call was made.
   In this way, file descriptors are usable, by default, in the process that originally created them,
   through the handle that was created at the time of the file descriptor's creation.
   This is the basic and simple case;
   a file descriptor is usable in the process that created it.
   
   Second, file descriptors can be inherited by creating a new process.
   When \texttt{clone} (without \verb|CLONE_FILES|) is called,
   it creates a new child process with a new file descriptor table,
   containing copies of all the file descriptors existing in the parent process at the time of the system call.
   These file descriptors can be used from the child process
   by creating handles for them using the \texttt{inherit\_fd} func
