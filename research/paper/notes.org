...
*** TODO non-OO APIs for other languages
...
*** kernel support
Several aspects of our implementation would be improved by kernel support.
Detecting the =execve= of a child process,
closing =CLOEXEC= file descriptors,
and inheriting =CLOEXEC= file descriptors explicitly specified as an =execve= argument
would all be helpful.
Implementing these kernel features in a generally useful way, and upstreaming them,
