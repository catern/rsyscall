...
demos, focused, and written!
*** introduction real
Processes, and the process interface for running programs, can be used for many purposes.
...
In Linux, these concepts were inherited as per-process namespaces.
...
But as expected,
as the current process creation interfaces are poor,
most applications don't use any of those features.
We don't claim that this is the sole reason,
but the poor quality of the process creation interface
is one thing blocking more advanced usage of processes.

We have adapted a common pattern which we see in other operating systems
to Unix.
In systems such as seL4 and KeyKOS,
a process is created by its parent in an inert state,
and then supplied with various resources,
and then started running.
The same mechanism that mutates a process while it is running,
is used to initialize the process while it is in an inert state.
In Unix, this corresponds to the parent operating on (calling the?) syscalls
We've termed this "direct-style process creation".
*** 
these features of processes are all used at process creation and initialization time.

hence it is important to have a good process creation and initialization interface.

our current such interfaces suck,
and as expected,
most systems would benefit from all these features.
yet most systems do not use any of them.
*** server paragraph
# hmm this is weak.
# I don't think most processes actually need servers.
# what am I really trying to go for here?
# I want each of my examples to be actually useful for every application.
# what is the server thing?
# it's maybe objects? it's maybe parallelism? 
# I mean, it is indeed parallelism and resources
# but it's also objects, a server is an object that hides its implementation
# aha! yes, implementation hiding, that's...
# that's something we've done before, drat!
# ok so, maybe we can do that hmm.
# well it's just the whole service-based thing.
# right, so maybe I should talk about some of the reasoning for services?
# like... modularity.
# i mean the abstraction thing is good.
# maybe we should go into more depth about abstraction?
# just have another section about it?
# actually, we should cut it short, we don't want too much verbosity.
# yeah let's cut it
Since processes run concurrently and can have different privilege levels and resources,
we can also create server processes which receive requests for operations on some resource
from less-privileged client processes.

This allows a less-privileged process to access constrained um.
*** introduction gist
...
*** outline 2
- features of processes, things they can do
- therefore it is important to have a good process creation interface
- currently widely available process creation interfaces suck for these reasons
- we have created a process creation interface that does not have these flaws

yes this is good.

then we go into describing the interface,
and then we give the examples

we don't have a separate background section.

I guess we can do all of this in the introduction?
** background
*** fork weaknesses
...
#+BEGIN_SRC python
child = thread.clone(CLONE.NEWUSER|CLONE.NEWNS)
...
* other papers
  cross-host operation with rsyscall
